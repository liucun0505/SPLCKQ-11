	...
	for (i = 0 ; i < nCeng + 1 ; i++)
	{
		...
		for(n = 0 ; n < nAlldataNumb ; n++)
		{
			if (_ttoi(strAllDatalist[n][4]) == i)
			{
				...
				m_fCoordinate[_ttoi(strAllDatalist[n][1])][1] = (i)* (m_cTBWeith + 2*m_cR)+TPSTART_Y_SHIFT;//获取节点Y坐标
				//获取节点X坐标
				if (_ttoi(strAllDatalist[n][4]) == 0)//层数为1时为根节点
				{
...
//--------------------------------------------------------------
	...
	for(int n = 0 ; n < nNumb ; n++)
	{
		...
		if (m_bSave)
		{
			...
			m_access.AddDataToTable(_T(".\\配置文件\\HistTPShow.accdb"), strData, strlistName, strlistData, 5);
		}
//--------------CSV 保存----------------------------------------
		if (m_bSave)
		{
			...
			(point.y > (m_fCoordinate[m_nPointNumb[n]][1] - m_cR/2))&&(point.y < (m_fCoordinate[m_nPointNumb[n]][1] + m_cR/2)))
		{
			if (!m_bShow)
			{
				...
				m_strPMessage = _T("MAC地址:") + strAllDatalist[m][0] + _T("\r\n") +
					...
					_T("层级:") + strAllDatalist[m][4];// +_T("%");//+ _T("\r\n") + 
