...
void CSimJzq::GetCOMMLENToBuf(INT16U  getLen , INT32U nWaitFramTime , INT16U nWaiteByteTime ,INT8U * recvbuf,INT16U &recvlen)
{
	INT16U len = 0;
	INT32U dlycnt=0;
	INT16U acklen;
...
INT8U CSimJzq::HostSendRcv376_2MsgAddrBuf(INT8U vSrcCn,INT8U * vSrcMsgAddr,INT8U vSrcAFN,INT16U vSrcFn,INT8U *vSrcBuf,INT16U vSrcLen,sPartQGDW376_2CreatFrame &sQGDW376_2HostFrame,TH_OUT sPartQGDW376_2DeCodeFrame &DstDecodeDataFrame, INT32U timeout,INT8U vSrcAckCmp)
{
	INT8U ack;
	...
	if(ack == DACK_SUCESS)
	{
		for(INT8U mm = 0; mm < 1; mm ++)
		{
			...
			if(vSrcAckCmp == 0)
			{
				ack = Wait3762AckCmpSeq(timeout , 50 ,sQGDW376_2HostFrame,DstDecodeDataFrame);
			}
			else
			{
				ack = Wait3762AckCmpAFNFN(timeout , 50 ,sQGDW376_2HostFrame,DstDecodeDataFrame);
			}
			if(ack == DACK_SUCESS)
			{
				break;
 ...
 INT8U CSimJzq::HostSendRcv376_2NoBuf(INT8U vSrcAFN,INT16U vSrcFn,sPartQGDW376_2CreatFrame &sQGDW376_2HostFrame,TH_OUT sPartQGDW376_2DeCodeFrame &DstDecodeDataFrame,INT8U vSrcAckCmp)
 {
	...
	if(ack == DACK_SUCESS)
	{
		for(INT8U mm = 0; mm < 1; mm ++)
		{
			...
			if(vSrcAckCmp == 0)
			{
				ack = Wait3762AckCmpSeq(20000 , 20 ,sQGDW376_2HostFrame,DstDecodeDataFrame);
			}
			else
			{
				ack = Wait3762AckCmpAFNFN(20000 , 20 ,sQGDW376_2HostFrame,DstDecodeDataFrame);
			}
			if(ack == DACK_SUCESS)
			{
				break;
...
INT8U CSimJzq::HostSendRcv376_2BufNode(TH_IN const INT8U *pSrcAddBuf, INT8U vSrcAFN,INT16U vSrcFn,INT8U *vSrcBuf,INT16U vSrcLen,sPartQGDW376_2CreatFrame &sQGDW376_2HostFrame,TH_OUT sPartQGDW376_2DeCodeFrame &DstDecodeDataFrame,INT8U vSrcAckCmp)
{
	...
	if(ack == DACK_SUCESS)
	{
		for(INT8U mm = 0; mm < 1; mm ++)
		{
			...
			if(vSrcAckCmp == 0)
			{
				ack = Wait3762AckCmpSeq(15000 , 40 ,sQGDW376_2HostFrame,DstDecodeDataFrame);
			}
			else
			{
				ack = Wait3762AckCmpAFNFN(15000 , 40 ,sQGDW376_2HostFrame,DstDecodeDataFrame);
			}
			if(ack == DACK_SUCESS)
			{
				break;
...
INT8U CSimJzq::HostSendRcv376_2F0F8MsgAddrBuf(INT8U vSrcCn,INT8U * vSrcMsgAddr, TH_IN const INT8U *pSrcAddBuf, TH_IN INT8U vSrcAfn,TH_IN INT16U vSrcFn,INT8U *vSrcBuf,INT16U vSrcLen,sPartQGDW376_2CreatFrame &sQGDW376_2HostFrame,TH_OUT sPartQGDW376_2DeCodeFrame &DstDecodeFrame,TH_OUT sPartQGDW376_2DeCodeFrame &DstDecodeDataFrame)
{
	...
	m_tools.Buf_To_LenBuf(ptSendQGDW376_2.s_SendDataBuf , ptSendQGDW376_2.s_SendDataLen , SendDatabuf , SendDatalen );
	INT8U ack = HostSendRcv376_2BufNode(pSrcAddBuf , 0xF0 , F8 , SendDatabuf , SendDatalen , sQGDW376_2HostFrame , DstDecodeFrame,1);
	switch(ack)
	{
	case 0:
		temp16 = DstDecodeFrame.s_RcvDataBuf[1]<<8 | DstDecodeFrame.s_RcvDataBuf[0];
...
void CSimJzq::BF_SendBufToCom(INT8U *vSrcBuf,INT16U vSrcLen)
{
	CMainFrame* pMainDlg= (CMainFrame*)AfxGetApp()->GetMainWnd();
	pMainDlg->BF_SendBufToCOMM(vSrcBuf ,vSrcLen , 0);	
	//plog.saveLog_HexBuf(DATA_FILE,OUTPUT,vSrcBuf,vSrcLen);//打印发送帧数据
...
INT8U   CSimJzq::Wait3762Ack(INT32U nWaitFramTime , INT16U nWaiteByteTime,TH_OUT sPartQGDW376_2DeCodeFrame &DstDecodeDataFrame)
{
	...
	if(recvbuflen>13)
	{
		...
		//nItem = pMainDlg->m_FrameShowWnd.m_myTabCtrl.m_ctrlMssage.GetItemCount();
		//pMainDlg->m_FrameShowWnd.m_myTabCtrl.m_ctrlMssage.InsertItem( nItem,pMainDlg-> m_strSysDateTime_rx);
		//pMainDlg->m_FrameShowWnd.m_myTabCtrl.m_ctrlMssage.SetItemText(nItem , 1, _T("接收<<"));
		//pMainDlg->m_FrameShowWnd.m_myTabCtrl.m_ctrlMssage.SetItemText(nItem , 2 , pMainDlg->m_tools._buf16tostr16(recvbuf , recvbuflen , true));
		//pMainDlg->m_FrameShowWnd.m_myTabCtrl.m_ctrlMssage.EnsureVisible(nItem,FALSE);
		//pMainDlg->m_FrameShowWnd.m_myTabCtrl.m_ctrlMssage.RedrawItems(nItem,nItem);
		nItem = pMainDlg->m_FrameShowWnd.m_myTabCtrl.m_ctrlFrame.GetItemCount();
		pMainDlg->m_FrameShowWnd.m_myTabCtrl.m_ctrlFrame.InsertItem( nItem,pMainDlg-> m_strSysDateTime_rx);
...
INT8U   CSimJzq::Wait3762AckCmpAFN(INT32U nWaitFramTime, INT16U nWaiteByteTime, sPartQGDW376_2CreatFrame sQGDW376_2HostFrame, TH_OUT sPartQGDW376_2DeCodeFrame &DstDecodeDataFrame)
{
	INT8U reack = DACK_FAILURE;
	INT8U ack = Wait3762AckCmpSeq(nWaitFramTime, nWaiteByteTime, sQGDW376_2HostFrame, DstDecodeDataFrame);
	if (ack == DACK_SUCESS)
	{
		if ((sQGDW376_2HostFrame.s_head.s_AFN == DstDecodeDataFrame.s_head.s_AFN))
		{
			reack = DACK_SUCESS;
		}
		else
