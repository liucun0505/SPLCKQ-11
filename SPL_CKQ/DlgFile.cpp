...
IMPLEMENT_DYNAMIC(Dlgfile, CDialogEx)

Dlgfile::Dlgfile(CString stWhere,CWnd* pParent /*=NULL*/)
: CDialogEx(Dlgfile::IDD, pParent)
{
	m_stWhere = stWhere;
}
...
void Dlgfile::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(Dlgfile, CDialogEx)
	ON_BN_CLICKED(IDC_BUTTON_OPEN_1, &Dlgfile::OnBnClickedButtonOpen)
	ON_BN_CLICKED(IDCANCEL_1, &Dlgfile::OnBnClickedCancel)
	ON_BN_CLICKED(IDOK_1, &Dlgfile::OnBnClickedOk)
END_MESSAGE_MAP()
...
void Dlgfile::OnBnClickedButtonOpen()
{
	// TODO: 在此添加控件通知处理程序代码
	UpdateData();
...
void Dlgfile::OnBnClickedCancel()
{
	// TODO: 在此添加控件通知处理程序代码
	CDialogEx::OnCancel();
}
...
void Dlgfile::FileNameIndexChange()
{
	SYSTEMTIME st;
	CString date=_T("");
...
void Dlgfile::SaveInfo(CString strTopLine)
{
...
void Dlgfile::SetFileName(CString str)
{
	m_sfilename = str;
}

void Dlgfile::ReadFileLine()
{
	char str[256];
	CString cstr;
...
void Dlgfile::SplitLine(CString cmdline,CString token)
{
